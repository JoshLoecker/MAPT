...
rule otu_from_filter_id_reads:
    ...
    run:
        ...
        for index, (in_file, out_file) in enumerate(zip(input_files, output_files)):
            ...
            Use set to get unique barcodes/clusters
            """
            barcode_rows = natural_sort([f"bc{item}" if item != "unclassified" else item for item in set(divergence_df["barcode"])])
            cluster_columns = natural_sort([f"otu{item}" for item in set(divergence_df["cluster"])])
            cluster_columns.insert(0,"barcode")
